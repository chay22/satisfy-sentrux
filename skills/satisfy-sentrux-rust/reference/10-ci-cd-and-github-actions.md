# 10 - CI/CD Integration & Quality Gating
...
Here is why:
1. **Source of Truth is `rules.toml`**: The definitive architectural law lives in `.sentrux/rules.toml` (which *is* committed to git). It enforces hard ceilings (`max_cycles = 0`, `max_cc = 10`, `[[layers]]`, `[[boundaries]]`, and `min_quality = 8500`).
...
Add this to `.github/workflows/ci.yml` (or `.github/workflows/sentrux.yml`):
...
```yaml
      - name: Ephemeral Regression Gate
        ...
        run: |
          # 1. Stash current PR changes and switch to target main
          git checkout main
          sentrux gate . --save # Creates ephemeral baseline on the fly
