...
Sentrux is a static analysis and structural architecture quality engine that enforces modularity, dependency constraints, cyclomatic complexity limits, and layering boundaries. It evaluates codebases using a multi-dimensional graph analysis and outputs a compound **Quality Signal** score capped at **10,000**.
...
| **Redundancy** | **9,500+** | $10,000 \times (1.0 - \text{Duplication Ratio})$ | Penalizes clone groups and duplicate logic. |
...
- **< 7,000**: Monolithic, entangled dependencies, frequent circular imports, god functions ($\text{CC} \ge 15$).
- **7,000 – 7,999**: Basic layering, some lingering import hubs, wide variation in function complexity ($\text{Gini} > 0.30$).
- **8,000 – 8,499**: Clean acyclic DAG, architectural boundary rules passing, no functions with $\text{CC} > 5$.
- **8,500 – 8,999**: Extreme structural purity: zero functions with $\text{CC} \ge 3$, modular router delegates, deduplicated utilities, perfect acyclicity ($10,000$).
